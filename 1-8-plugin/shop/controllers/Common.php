...
class cart extends Fn_base {
    ...
    public function read_cart() {
        //从购物车cookie中读取数据
        $data = cookie::get($this->cart_name);
        if (!$data) {
            return false;
        }
        return $data;
     ...
     * @return boolean
     */
    public function add($id, $options = array()) {
        if (!$id) return false;
        $num  = (int)$options['num'];
        $data = $this->read_cart();
        //当购物车中没有商品记录时
        ...
        cookie::set($this->cart_name, $data, $this->life_time, $this->cookie_path);
        return true;
    }
    ...
    public function update_num($cartid, $num) {
        ...
        cookie::set($this->cart_name, $cart_data, $this->life_time, $this->cookie_path);
        return true;
    }
