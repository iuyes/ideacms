...
class MemberController extends Plugin {
	...
	public function addressAction() {
	    if ($this->isPostForm()) {
			...
			if ($form == 'add') {
			    $data = $this->post('data');
				if (empty($data['name']) || empty($data['tel']) || empty($data['address']) || empty($data['zip'])) $this->memberMsg('填写不完整！');
				$data['userid']   = $this->memberinfo['id'];
				$data['username'] = $this->memberinfo['username'];
				$id = $this->address->insert($data);
				if (!$id) $this->memberMsg('添加失败！');
			} else {
			    $delete = $this->post('delete');
				if (is_array($delete) && $delete) {
				    $this->address->delete('id IN (' . implode(',', $delete) . ')');
				}
	...
	public function payAction() {
	    if (!$this->setting['pay']) $this->memberMsg('无需要付款，等待客服与您联系！');
